...
class Game {
    ...
    Game() {
        _stock = new Pile("Stock", 70, 400, 0);
        _everything.add(_stock);
        _waste = new Pile("Waste", 185, 400, 0);
        _everything.add(_waste);
        _reserve = new Pile("Reserve", 70, 200, 0);
        _everything.add(_reserve);

        for (int i = 1; i <= TABLEAU_SIZE; i += 1) {
            int x = 300 + 115 * (i - 1);
            int y = 200;
            Pile tab = new Pile("Tableau", x, y, i);
            _tableau.add(tab);
            _everything.add(_tableau.get(i - 1));
        }
        for (int i = 1; i <= 4; i += 1) {
            int x = 300 + 115 * (i - 1);
            int y = 55;
            Pile foundation = new Pile("Foundation", x, y, i);
            _foundation.add(foundation);
            _everything.add(_foundation.get(i - 1));
        }
    ...
    /** store game state and push to the stack savedStates. */
    public void save() {
        Game copy = new Game();
        copy.copyFrom(this);
    ...
    /** Return pile thats in the distance of x,y. */

    Pile findPile(int x, int y) {
        for (Pile p : _everything) {
            if (p.includes(x, y)) {
                return p;
            }
        }
    ...
    /** all my piles. */
    private ArrayList<Pile> _everything = new ArrayList<>();

    /** stack to store each state. */
    private static Stack<Game> savedStates = new Stack<Game>();
