...
class Pile {
    ...
    /** name of the pile */
    String name;
    /** index used to indicate different tableau and foundation*/
    int index;
    /**constructor with location x,y, name, and index*/
    Pile(String nameofpile, int x, int y, int n) {
        name = nameofpile;
        xx = x;
        yy = y;
        index = n;
    ...
    /**return true if the point is (x,y) is in the range of this pile(xx,yy) */
    public boolean includes(int x, int y) {
        return xx <= x && x <= xx + 90 && yy <= y && y <= yy + 125;
    }
