...
def do_lambda_form(expressions, env):
    ...
    return LambdaProcedure(formals, Pair(body.first, nil), env)

def do_if_form(expressions, env):
    """Evaluate an if form."""
    check_form(expressions, 2, 3)
    "*** YOUR CODE HERE ***"
    checkif = scheme_eval(expressions.first, env)
    if scheme_true(checkif):
...
def do_and_form(expressions, env):
    ...
    for index in range(len(expressions) -1):
        if not scheme_true(scheme_eval(expressions[index], env)):
            return False
    return scheme_eval(expressions[len(expressions) -1], env)
...
def do_or_form(expressions, env):
    ...
    if len(expressions) == 0:
        return False
    for index in range(len(expressions) -1): 
        value=scheme_eval(expressions[index], env)
...
def make_let_frame(bindings, env):
    ...
    if len(bindings.first) > 2: 
        scheme_eval(bindings.first.first, env)
        raise SchemeError
    else:
        names, values = nil, nil
        for b in range(len(bindings)):
            names = Pair(bindings[b].first, names)
            values = Pair(scheme_eval(bindings[b].second.first, env), values)
...
def do_mu_form(expressions, env):
    ...
    if len(expressions) > 2:
        body = do_begin_form(formals,env)
    else:
        body = expressions.second[0]
