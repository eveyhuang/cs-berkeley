...
(define (zip pairs)
  (cond
    ( (null? pairs) '(()()) )
    ( (null? (cdr pairs)) (cons (list (caar pairs)) (list (cdr(car pairs)))) )
    ( else (cons (car (cons-all (caar pairs) (zip (cdr pairs)))) 
                 (cdr (cons-all (cadar pairs) (zip (cdr pairs))))))
  ))
...
(define (list-partitions total)
  (define (partition-helper total total2 ) 
    (cond ((= total2 0) nil)
          ((= total 0) (list nil))
          ((< total 0) nil)
          
          (else (append (apply-to-all 
            (lambda (x) (cons total2 x)) 
            (partition-helper (- total total2) total2 )) 
            (partition-helper total (- total2 1)  )))))

  (define (filter element)
    (cond
      ( (null? element)  nil )
      ( (null? (cdr element)) element)
      ( (not (list? (cdr element))) (cond 
                                    ((= (car element) (+ 1 (cdr element)))  nil )
                                    ( else element)
                                      )  )
      ( (= (car element) (+ 1 (cadr element)) )  nil )
      ( else (cons (car element) (filter (cdr element))))
    ))  

    
  
    (define a (partition-helper total total ))

     (apply-to-all filter a) 
  ...
  (cond ((atom? expr)
        ...
        ((let? expr)
         (let ((values (cadr expr))
               (body   (cddr expr)))
           (lambda (analyze body) (analyze values))
           ))
        (else
         'YOUR-CODE-HERE
         )))
