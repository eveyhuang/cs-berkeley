...
def find_closest(location, centroids):
    ...
    """
    "*** YOUR CODE HERE ***"
    d = {n:distance(location, centroids[n]) for n in range(0, len(centroids))}
    a= min(d, key=d.get)
    return centroids[a]
...
def group_by_centroid(restaurants, centroids):
    ...
    for element in restaurants:
        location = restaurant_location(element)
        centroidlist.append(find_closest(location, centroids))
    return group_by_first(zip(centroidlist, restaurants))
...
def find_centroid(restaurants):
    ...
    lon = []
    for element in restaurants:
        lat.append(restaurant_location(element)[0])
        lon.append(restaurant_location(element)[1])
...
def find_predictor(user, restaurants, feature_fn):
    ...
    #the extracted values for each restaurant
    ...
    for i in xs:
        ssum.append(i-mx)
        sxxx.append(pow(i - mx, 2))
    for n in ys:
        ysum.append(n - my)
        syyy.append(pow(n - my, 2))
    from operator import mul
    sxy= sum(map(mul, ssum, ysum))
    sxx= sum(sxxx)
    syy= sum(syyy) 
    b= sxy/sxx
    a= my-(sxy/sxx)*mx
    r_squared= pow(sxy,2)/(sxx*syy)  # REPLACE THIS LINE WITH YOUR SOLUTION
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for i in feature_fns:
        predictors, r_value=find_predictor(user, reviewed, i)
        predandr[predictors]=r_value
    a_best= max(predandr, key=predandr.get)
    return a_best
...
def rate_all(user, restaurants, feature_functions):
    ...
    "*** YOUR CODE HERE ***"
    reviewed = user_reviewed_restaurants(user, restaurants).keys()
    predictor = best_predictor(user, RESTAURANTS, feature_functions)
    rateall= {}
    for n in restaurants.keys():
        if n in reviewed:
            rateall[n]= int(user_rating(user, n))
        else:
            rateall[n] = predictor(restaurants[n])
    return rateall
